...
[STEP3/3] 事後確認
  └─ run_blsw_check()（全 Leaf 完了後に一括）
      ├─ 各対象ノードの area_network を取得
      ├─ Grafana で BL-SW トラフィック確認（IN/OUT 現在値 >= 基準値）
      └─ NG のノードを each_status_code=E に更新
          （既に E のノードはスキップ、判定不能/area 取得不可は E）
...
[STEP4/4] 事後確認
  ...
  ├─ all_ok（admin/oper/module/diag 等）判定
  ├─ all_ok が True の場合のみ BL-SW トラフィック確認（run_blsw_check 相当を直接実行）
  └─ 判定（3 分岐）
      ├─ all_ok かつ BL-SW OK       → 正常終了
      ├─ all_ok かつ BL-SW NG       → each_status_code=E（BL-SW NG）
      └─ all_ok が False            → each_status_code=E（事後確認 NG）
```
...
[STEP4/N] 事後確認
  └─ run_blsw_check()（全ノード完了後に一括、leaf/spine 共通）
      ├─ 各対象ノードの area_network を取得
      ├─ Grafana で BL-SW トラフィック確認
      └─ NG のノードを each_status_code=E に更新
...
| `get_area_network(hostname)` | DB から hostname の area_network のみを取得（BL-SW 確認用、取得不可なら None） |
...
- `GRAFANA_CHECK_ENABLED=False` の場合は何もしない
- 既に `each_status_code=E` のノードはスキップ（原因メッセージを上書きしない）
...
### 7.4 BL-SW トラフィック確認（`blsw_traffic_check` / `run_blsw_check`）

切り離し・組み込みの事後確認として、BorderLeaf-SpineSwitch 間の総トラフィックが
正常に流れているかを Grafana（Zabbix データソース）で確認する。ACI の状態確認
（admin/oper）に加えた、実トラフィックの裏取り。
...
| シナリオ | ノード種別 | タイミング |
|---|---|---|
| Disable | Leaf | 全 Leaf 完了後に一括（`run_blsw_check`） |
| Disable | Spine | 各ノードの事後確認内（`all_ok` 判定に統合） |
| Enable | Leaf / Spine | 全ノード完了後に一括（`run_blsw_check`） |

NG 時の扱い：

- 該当ノードの `each_status_code` を `E` に更新
- `finalize_status` により全体 `status_code` は `異常終了を含む`（先頭 N）となる
- 既に E のノードは上書きしない
- 判定不能（Grafana 接続失敗・ダッシュボード未検出・データ無し）や
  `area_network` 取得不可は、安全側として E にする

ON/OFF：`credentials.GRAFANA_CHECK_ENABLED` で切り替え。`False` の場合は確認を
スキップし、ログに「無効(スキップ)」とだけ記録する。
...
| Enable の ID 不明（`run/<uid>` 不在） | `fail_all_and_exit` で全体停止 |
...
**エラー処理の原則**：一度ノード処理ループに入った後は、個別ノードの失敗で全体を停止せず、該当ノードのみ異常終了として記録し次のノードへ進む。全体を停止（`fail_all_and_exit`）するのは、ループに入る前の前提条件エラー（最初の APIC 接続、Enable の対象データ不在）のみ。これにより、一部ノードの失敗が既に成功したノードの結果を巻き込むことを防ぐ。
